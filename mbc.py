...
def get_target_setting(controlDict, nodes):
    ...
    for i in controlDict:
        ...
        if complicated == True and pump == False:
            if controlDict[i]['shape'] == 'RECT_CLOSED':
                ...
                y = 4 * controlDict[i]['q_goal'] / ( controlDict[i]['Cd'] * np.sqrt(2*g) * controlDict[i]['geom1']**2 * np.pi )
                answer = np.roots([controlDict[i]['geom1']/2,h1 - upstream.invert_elevation,0,0,0,y**2])

            else:
                print(i, "got in to the complicated solver, but isn't an orifice.")

            for number in answer:
                if np.real(number) < 1.0 and np.real(number) > 0.0:
                    controlDict[i]['action'] = np.real(number)

        elif complicated == False and pump == False:
            # complicated is false and still orifice
            if np.allclose(0.0,H,atol=1e-04) or H < 0.0:
                ...
                if controlDict[i]['shape'] == 'CIRCULAR':
                    target_setting = ( 4 * controlDict[i]['q_goal'] / (controlDict[i]['Cd'] * np.sqrt(2 * g * H) * controlDict[i]['geom1']**2 * np.pi) ) **(1/2)
                    controlDict[i]['action'] = target_setting
