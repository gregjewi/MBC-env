...
def mask_to_dict(mask,dict):
    dict_of_mask = {}
    for foo,key in zip(mask,dict):
        if foo == True:
            dict_of_mask[key] = dict[key]
            
    return dict_of_mask
        
def get_group(group,controlDict,upstreamDict,downstreamDict):
    
    C_mask = [controlDict[key]['group'] == group for key in controlDict] 
    U_mask = [upstreamDict[key]['group'] == group for key in upstreamDict] 
    D_mask = [downstreamDict[key]['group'] == group for key in downstreamDict] 
    
    C_fill = mask_to_dict(C_mask,controlDict)
    U_fill = mask_to_dict(U_mask,upstreamDict)
    D_fill = mask_to_dict(D_mask,downstreamDict)
